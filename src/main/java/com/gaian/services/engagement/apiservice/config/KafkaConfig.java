...
import com.gaian.services.engagement.apiservice.kafka.DeliveryRequestDeserializer;
...
import com.gaian.services.engagement.apiservice.kafka.ApiTaskSubscriber;
...
public class KafkaConfig {
	...
	@Autowired
	private ApiTaskSubscriber subscriber;
	
	@Autowired
	private KafkaConsumerFactory consumerFactory;
	
	@Value("${kafka.apiTaskConsumer.topic}")
	private String engagementTopic;
	
	private Map<String, String> apiTaskConsumer;
     ...
     */
	 ...
	 @PostConstruct
	    public void setUpEngagementEventListeners() {
		 ...
		 apiTaskConsumer.put("key.deserializer", StringDeserializer.class.getName());
		 apiTaskConsumer.put("value.deserializer", DeliveryRequestDeserializer.class.getName());

	        Integer concurrency = ofNullable(apiTaskConsumer.get("concurrency"))
	            .map(Integer::valueOf).orElse(30);

	        range(0, concurrency).forEach(index ->

	            consumerFactory.<String, DeliveryRequest>create(apiTaskConsumer).doOnVertxConsumer(
	                consumer -> consumer.subscribe(engagementTopic, subscription -> {
	                    ...
	                    if (subscription.succeeded()) {
	                        ...
	                        Long minPollTime = ofNullable(apiTaskConsumer.get("min.poll.interval.ms"))
	 ...
	 public void pollMessages(
		        ...
		        Long pollTimeout = ofNullable(apiTaskConsumer.get("poll.timeout.ms"))
	 ...
	 private void commit(
		        ...
		        if (!ofNullable(apiTaskConsumer.get("enable.auto.commit"))
