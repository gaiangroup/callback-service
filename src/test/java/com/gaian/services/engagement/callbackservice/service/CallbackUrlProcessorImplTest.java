...
class CallbackUrlProcessorImplTest {
	...
	@InjectMocks
	CallbackUrlProcessorImpl callbackUrlProcessorImpl;
	...
	public void resumeEngagementTest() {
		...
		CallbackUrl callbackChannel = new CallbackUrl();
		callbackUrl.setUrlType("REST");
		callbackUrl.setUrl("https://ingress-gateway.gaiansolutions.com/darpa/v1.0/{id}/details/{size}");
		callbackUrl.setHttpMethod(HttpMethod.POST);
		callbackUrl.setHeaders(headers);
		callbackUrl.setPathParams(pathParams);
		callbackUrl.setQueryParams(queryParams);
		callbackUrl.setBody(body);
		callbackUrl.setDataSources(dataSources);
		callbackUrl.getChannelType();

		ChannelType channelType = callbackUrl.getChannelType();



		EngagementDBModel engagementDBModel = new EngagementDBModel();

		engagementDBModel.setChannel(callbackUrl);
